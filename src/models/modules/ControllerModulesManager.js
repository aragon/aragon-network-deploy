...
module.exports = class extends BaseDeployer {
  ...
  async setModules() {
    ...
    await this.court.setModules(ids, addresses)
    logger.success('Modules set successfully')
  }

  async setModulesThroughDAO() {
    const { controller, governor: { modules: dao } } = this.config
    ...
    const executeData = this.encoder.encodeExecute(controller, 0, setModulesData)
